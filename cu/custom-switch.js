class CustomSwitch extends HTMLElement {
  ...
  bindEvents() {
    ...
    let didDrag = false; // 用于判断是否发生了拖动
    
    switchElement.addEventListener('click', (event) => {
      if (this._disabled || isDragging || didDrag) return; // 添加didDrag检查
      this.toggleSwitch();
      // didDrag会在下一次startDrag时重置
    });
    ...
    const startDrag = (event) => {
      ...
      isDragging = true;
      didDrag = false; // 重置拖动标志
      initialX = event.clientX || event.touches[0].clientX;
      initialLeft = parseInt(window.getComputedStyle(slider).left, 10);
      
      document.addEventListener('mousemove', onMouseMove);
      document.addEventListener('touchmove', onMouseMove, { passive: false }); // 允许preventDefault
    ...
    const onMouseMove = (event) => {
      ...
      if (this._variant === 'blur') {
        ...
        // 修改颜色计算逻辑，取消强制白色，平滑过渡颜色和透明度
        const offColor = [94, 114, 228]; // 蓝紫色起始颜色
        const onColor = [0, 123, 255];   // 蓝色结束颜色
        const secondGradientColor = [0, 255, 123]; // 绿色渐变色
        
        // 根据百分比计算颜色
        const currentColor = offColor.map((off, index) => {
          return Math.round(off + (onColor[index] - off) * percentage);
        });
        
        // 透明度也随位置变化，从0.1到0.3
        const startOpacity = 0.1;
        const endOpacity = 0.3;
        const currentOpacity = startOpacity + (endOpacity - startOpacity) * percentage;
        
        // 第二个渐变色的透明度也随位置变化
        const secondOpacity = 0.1 + 0.2 * percentage;
        
        switchElement.style.transition = 'none';
        switchElement.style.background = `linear-gradient(145deg, 
          rgba(${currentColor.join(',')}, ${currentOpacity}), 
          rgba(${secondGradientColor.join(',')}, ${secondOpacity}))`;
        
        // 调整光效强度，使其也平滑过渡
        const glowIntensity = 0.1 + 0.5 * percentage;
        slider.style.boxShadow = `0 8px 15px rgba(0, 0, 0, 0.2), 
          0 0 20px rgba(0, 123, 255, ${glowIntensity}), 
          inset 0 0 8px rgba(255, 255, 255, ${0.4 + 0.2 * percentage})`;
      }
    };
    ...
    const onMouseUp = () => {
      ...
      switchElement.style.transition = '';
      ...
      setTimeout(() => {
        // 再次确认滑块位置是否正确
        slider.style.left = this.checked ? '31px' : '1px';
      }, 0);
      
      this.dispatchChangeEvent();
      
      // 必须在所有逻辑处理完之后再设置 isDragging = false
      isDragging = false;
      
      // 添加延时重置didDrag标志，使点击事件可以正常工作
      setTimeout(() => {
        didDrag = false;
      }, 300); // 300毫秒后重置，避免与可能的点击事件冲突
    };
    ...
    slider.addEventListener('click', (event) => {
      ...
      if (!this._disabled && !isDragging && !didDrag) { // 添加didDrag检查
        this.toggleSwitch();
      }
    });
    ...
    // 防止iOS上的触摸呼出菜单
    slider.style.webkitTouchCallout = 'none';
  }
